...
class NeuralNetwork(tf.keras.models.Sequential):
    def __init__(self,learning_rate=0.05):
        super().__init__()
        #input layer, batch size =1 for the replay memory, number states as inputs (16)
        self.add(tf.keras.layers.InputLayer(batch_input_shape=(1,16)))
        #hidden layer
        self.add(tf.keras.layers.Dense(64,activation='sigmoid'))
        #output layer
        self.add(tf.keras.layers.Dense(4,activation='linear'))
    ...
    def train(self,state,target_output):
        ...
        target_output = target_output.reshape(-1,4)
        #fit ?
